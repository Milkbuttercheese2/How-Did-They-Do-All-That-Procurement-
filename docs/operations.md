...
## 한도 자동 재개 프로토콜 (2026-07-17 제도화 — 운영자 지시)

법령 콘텐츠 제작·검토는 토큰 한도에 걸려도 사람 개입 없이 **한도 초기화 시각에 자동 재개**한다. 세션을 운영하는 에이전트는 아래를 표준 절차로 따른다:

1. **감지**: 서브에이전트 실패 메시지의 `You've hit your session limit · resets N(am/pm) (UTC)`에서 리셋 시각을 읽는다. 워크플로 결과의 `failures` 목록에 한도 에러가 하나라도 있으면 발동.
2. **체크포인트**: 이미 파일에 적용된 정정·작성분을 즉시 검사(JSON 파싱, current 노드 1개) 후 작업 브랜치에 커밋·푸시한다. 완료·검증분만 배포 대상(95% 규칙과 동일 원칙).
3. **재개 예약**: `send_later`(claude-code-remote MCP)로 **리셋 시각 +5분**에 이 세션을 깨우는 메시지를 예약한다. 메시지에는 반드시 다음을 담는다: 재개할 워크플로의 `scriptPath`+`resumeFromRunId`+원본 `args`(또는 그 위치), 체크포인트 커밋 해시, 남은 통합·배포 단계. (send_later는 컨테이너 회수 후에도 세션을 되살린다.)
4. **재개 실행**: 깨어나면 워크플로를 `resumeFromRunId`로 재가동한다 — 완료 에이전트는 캐시로 즉시 복원되고 실패 단계만 실제 재실행된다. 재개 후에도 50~60분 간격 자기 점검(send_later)을 재무장해 중단 상태로 방치되지 않게 한다.
5. **반복**: 재개 라운드가 또 한도에 걸리면 1~4를 반복한다. 최종 완료 시에만 기계 게이트→머지→배포→푸시 알림.

주의: 예약 메시지·프롬프트 어디에도 자격증명 값(LAW_OC 등)을 적지 않는다(환경변수 참조만).
