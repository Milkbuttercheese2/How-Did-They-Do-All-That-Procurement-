...
**🔗 라이브 사이트: <https://milkbuttercheese2.github.io/procurement-system-100/>**
...
- 제도 66개가 공개되어 있습니다(등록·발주·공고·입찰·심사평가·계약이행·사후관리제재의 조달 업무 순서로 배열). 제도별 검증 상태는 상세 페이지 배지로 표시합니다.
- 이 가운데 조문 기계 대조(국가법령정보센터 Open API)를 통과한 제도는 `article-verified` 배지로, 출처는 연결했으나 조문 기계 대조가 남은 제도는 `needs-review` 배지로 구분해 표시합니다. `needs-review` 제도는 재검증 대상으로 관리합니다.
- 검증 배치 3~6에서 조문 기계 대조가 남은 초안(계약서 작성, 시설공사 조달, 가격조사·원가계산, 품질관리, 종합쇼핑몰 구매)은 검증 완료 전까지 `sources/batch-2026-07-13/drafts/`에 보류하고 공개 빌드에 포함하지 않습니다.
...
- 접속 주소: <https://milkbuttercheese2.github.io/procurement-system-100/>
