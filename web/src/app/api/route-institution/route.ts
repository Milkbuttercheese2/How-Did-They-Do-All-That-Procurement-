...
// 실무 약칭·영문 약어는 제도 이름과 글자가 안 겹쳐 프리필터에서 밀린다.
// 측정해보니 "종심제"는 0.125로 하한(0.12)에 간신히 걸치고, "턴키"·"PQ"는
// 엉뚱한 제도가 1위로 올라왔다. 질의를 정식 명칭으로 넓혀 매칭을 돕는다.
// (법령정보 MCP가 약칭을 자동 변환하는 것과 같은 취지)
const SYNONYMS: Array<[RegExp, string]> = [
  [/국계법/g, "국가계약법 국가를 당사자로 하는 계약"],
  [/지계법/g, "지방계약법"],
  [/종심제/g, "종합심사낙찰제 낙찰자 결정"],
  [/적심|적격심사/g, "적격심사낙찰제 낙찰자 결정"],
  [/턴키/g, "대형공사 일괄입찰 대안입찰"],
  [/PQ/gi, "입찰참가자격 사전심사"],
  [/MAS/gi, "다수공급자계약"],
  [/나라장터|종합쇼핑몰/g, "국가종합전자조달 전자입찰 종합쇼핑몰 구매"],
  [/방폐|방위사업/g, "방위사업 계약"],
  [/하도급/g, "하도급 관리 하도급대금 직접지급"],
  [/지체상금|지연배상/g, "지체상금 이행 지체"],
  [/부정당|제재/g, "부정당업자 입찰참가자격 제한"],
  [/설변|설계변경/g, "설계변경 계약금액 조정"],
  [/물가|에스컬|자재값|자재비/g, "물가변동 계약금액 조정"],
  [/유찰|재공고/g, "재입찰 재공고 유찰 처리"],
  [/선금|선급금/g, "선금 대가 지급"],
  [/검수|납품검사/g, "검사 검수"],
];
...
function expand(query: string): string {
  ...
  for (const [pattern, extra] of SYNONYMS) {
    if (pattern.test(query)) out += ` ${extra}`;
    pattern.lastIndex = 0;
  }
  return out;
}
